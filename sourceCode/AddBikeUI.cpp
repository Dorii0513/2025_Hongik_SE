...
/*
	함수 이름 : AddBikeUI 
	기능	  : AddBikeUI boundary 클래스 생성자
	전달 인자 : AddBike* ctrl -> AddBike control 클래스
	반환값    : 없음
*/
...
/*
	...
	기능	  : input.txt 파일의 input 값을 전달 받고, 
                이를 control 클래스로 전달하여 addNewBike 메서드를 호출합니다.
                이후 자전거가 등록되었다는 ouput 값을 출력합니다.
...
void AddBikeUI::addBike(std::ifstream& in_fp, std::ofstream& out_fp){
    ...
    control->addNewBike(bikeID, bikeName);
